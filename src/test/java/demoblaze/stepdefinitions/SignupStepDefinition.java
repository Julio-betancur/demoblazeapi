...
import demoblaze.tasks.SendSignup;
import demoblaze.tasks.ValidateSignup;
...
public class SignupStepDefinition {
    @DataTableType
    public SignupDataTable convert(Map<String, String> table) {
        return new ObjectMapper().convertValue(table, SignupDataTable.class);
    }
    ...
    public void iSendTheUserDetails(SignupDataTable data) {
        actor.attemptsTo(
                SendSignup.withInformation(data)
        );
    }
    @Then("I validate the response content for the signup")
    public void iValidateTheResponseContentForTheSignup(SignupDataTable data) {
        actor.attemptsTo(
                ValidateSignup.response(data)
        );
    }
}
